...
namespace FNRC_DigitalHub.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class WhoAmIController(IConfiguration config) : BaseController
    {
        ...
        public async Task<IActionResult> GetCurrentUserAsync()
        {
            ...
            await CreateUserSessionAsync(samAccountName, displayName, displayName, 0, UserType, "", "");
        ...
        private async Task CreateUserSessionAsync(string username, string nameEn, string nameAr, int employeeId, ICollection<UserTypeDTO> userTypes, string mobileNo, string email)
        {
                ...
                var claims = new List<Claim>
                    {
                        ...
                        new (ClaimTypes.Role, data.Type.ToString()),
                       // new  (ClaimTypes.Authentication, data.Token),
                        new  ("Type", data.Type.ToString()),
                        //new  ("UserID", data.User.Id.ToString()),
                        //new  ("IsChangePassword", data.User.IsChangePassword.GetValueOrDefault().ToString()),
                    };
